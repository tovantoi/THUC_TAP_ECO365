...
namespace _365EJSC.ERP.Persistence.Repositories.Define
{
    ...
    public class WebLocalDistrictSqlRepository : GenericSqlRepository<WebLocalDistrict, int>, IWebLocalDistrictSqlRepository
    {
        ...
        public async Task<WebLocalDistrict?> FindSingleAsync(Expression<Func<WebLocalDistrict, bool>>? predicate, bool isTracking = false, CancellationToken cancellationToken = default, params Expression<Func<WebLocalDistrict, object>>[] includeProperties)
        {
            WebLocalDistrict? webLocalDistrict = await base.FindSingleAsync(predicate, isTracking, cancellationToken, includeProperties);

            if (webLocalDistrict is not null)
            {
                CustomException.ThrowNotFoundException(typeof(WebLocalDistrict), MsgCode.INF_FOUND, WebLocalDistrictConst.MSG_DISTRICT_PROVINCE_EXIST);
            }

            return webLocalDistrict;
        }
    }
