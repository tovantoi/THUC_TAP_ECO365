...
namespace _365EJSC.ERP.Presentation.Controllers.v1.Define
{
    ...
    /// </summary>
    [ApiVersion(1)]
    [Route(RouteConstant.API_PREFIX + RouteConstant.WEBLOCAL_PROVINCE_ROUTE)]
    public class WebLocalProvinceController : ApiController
    {
        private readonly IMediator mediator;

        public WebLocalProvinceController(IMediator mediator)
        {
        ...
        [MapToApiVersion(1)]
        [HttpPost]
        public async Task<IActionResult> CreateV1(CreateWebLocalProvinceRequest request)
        {
        ...
        [MapToApiVersion(1)]
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateV1(int id, [FromBody] UpdateWebLocalProvinceRequest request)
        {
            request.Id = id;
            var result = await mediator.Send(request);
            return Ok(result);
        ...
        {
            var command = new DeleteWebLocalProvinceRequest()
            {
                Id = id,
            };
        ...
        {
            var query = new GetDetailWebLocalProvinceRequest()
            {
                Id = id,
            };
        ...
        [MapToApiVersion(1)]
        [HttpGet]
        public async Task<IActionResult> GetAllV1([FromQuery] string? localization)
        {
            var request = new GetAllWebLocalProvinceRequest() { KeyLocalization = localization };
            var result = await mediator.Send(request);
            return Ok(result);
        }
    }
