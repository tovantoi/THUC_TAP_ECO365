...
namespace _365EJSC.ERP.Presentation.Controllers.v1.Define
{
    ...
    /// </summary>
    [ApiVersion(1)]
    [Route(RouteConstant.API_PREFIX + RouteConstant.WEBLOCAL_WARD_ROUTE)]
    public class WebLocalWardController : ApiController
    {
        private readonly IMediator mediator;

        public WebLocalWardController(IMediator mediator)
        {
        ...
        [MapToApiVersion(1)]
        [HttpPost]
        public async Task<IActionResult> CreateV1(CreateWebLocalWardRequest request)
        {
        ...
        [MapToApiVersion(1)]
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateV1(int id, [FromBody] UpdateWebLocalWardRequest request)
        {
            request.Id = id;
            var result = await mediator.Send(request);
            return Ok(result);
        ...
        {
            var query = new GetDetailWebLocalWardRequest()
            {
                Id = id,
            };
        ...
        [MapToApiVersion(1)]
        [HttpGet]
        public async Task<IActionResult> GetAllV1([FromQuery] int? districtId)
        {
            var request = new GetAllWebLocalWardRequest() { DistrictId = districtId };
            var result = await mediator.Send(request);
            return Ok(result);
        }
    }
