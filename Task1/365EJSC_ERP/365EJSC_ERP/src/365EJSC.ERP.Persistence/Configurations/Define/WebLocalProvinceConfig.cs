...
namespace _365EJSC.ERP.Persistence.Configurations
{
    ...
    public class WebLocalProvinceConfig : IEntityTypeConfiguration<WebLocalProvince>
    {
        public void Configure(EntityTypeBuilder<WebLocalProvince> builder)
        {
            ...
            builder.Property(x => x.Latitude).HasColumnName(WebLocalProvinceConst.FIELD_LATITUDE).HasColumnType("float");
            builder.Property(x => x.Longitude).HasColumnName(WebLocalProvinceConst.FIELD_LONGITUDE).HasColumnType("float");
            ...
            builder.HasOne(x => x.WebLocals)
                        .WithMany()
                        .HasForeignKey(w => w.KeyLocalization);
